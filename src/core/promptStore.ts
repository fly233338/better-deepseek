...
const RETIRED_BUILTIN_TITLES = new Set([
  '角色扮演', '中英互译', '学习导师', '深度分析', '头脑风暴', '周报与汇报', '方案对比',
]);

const RETIRED_BUILTIN_FINGERPRINTS = new Set([
  computeFingerprint('角色扮演', '你将扮演{{角色名称}}。以下是你的设定：\n\n{{角色设定}}\n\n现在请以这个角色的身份回复：{{用户输入}}'),
  computeFingerprint('中英互译', [
    '请将下面内容翻译为{{目标语言}}。',
    '',
    '要求：',
    '1. 保留原文格式、列表和代码块。',
...
const BUILTIN_PROMPTS: (Omit<PromptItem, 'id' | 'createdAt' | 'updatedAt' | 'usageCount'> & { fingerprint: string })[] = [
  {
    title: '提问优化器',
    description: '把模糊问题改写成更容易得到高质量回答的提示词',
    content: [
    ...
    favorite: true,
    source: 'builtin',
    fingerprint: computeFingerprint('提问优化器', [
      '请帮我优化下面这个问题，让它更清晰、更具体、更适合交给 AI 回答。',
      '',
      '要求：',
      '1. 先指出原问题中缺失的信息或可能的歧义。',
  ...
  },
  {
    ...
    favorite: true,
    source: 'builtin',
    fingerprint: computeFingerprint('写作润色', [
      '请润色下面的文本。',
      '',
      '目标风格：{{目标风格}}',
      '读者对象：{{读者对象}}',
  ...
  },
  {
    ...
    favorite: false,
    source: 'builtin',
    fingerprint: computeFingerprint('总结提炼', [
      '请总结下面内容。',
      '',
      '输出格式：',
      '1. 3-5 条核心结论。',
  ...
  },
  {
    ...
    favorite: true,
    source: 'builtin',
    fingerprint: computeFingerprint('代码审查', [
      '请以资深工程师的方式审查下面的代码。',
      '',
      '背景/目标：{{背景}}',
      '',
  ...
  },
  {
    ...
    favorite: false,
    source: 'builtin',
    fingerprint: computeFingerprint('角色', [
      '请帮我创建一个可用于 AI 角色扮演的角色卡。',
      '',
      '角色方向：{{角色方向}}',
      '互动风格：{{互动风格}}',
...
export class PromptStore {
  ...
  private upgradeBuiltins(now: number): PromptItem[] {
    ...
    for (const prompt of this.data.prompts) {
      if (prompt.source !== 'builtin') continue;
      if (!RETIRED_BUILTIN_TITLES.has(prompt.title)) continue;

      const currFingerprint = prompt.fingerprint ?? computeFingerprint(prompt.title, prompt.content);
      const unchanged = RETIRED_BUILTIN_FINGERPRINTS.has(currFingerprint);
      const used = prompt.usageCount > 0 || prompt.favorite || Boolean(prompt.lastUsedAt);

      if (used || !unchanged) {
        prompt.source = 'imported';
        prompt.sourceName = '旧版内置';
        prompt.updatedAt = now;
        upgraded.push({ ...prompt });
      } else {
        const idx = this.data.prompts.indexOf(prompt);
        if (idx >= 0) this.data.prompts.splice(idx, 1);
      }
    }
    ...
    for (const [index, builtin] of BUILTIN_PROMPTS.entries()) {
      ...
      if (existing) {
        ...
        existing.fingerprint = builtin.fingerprint;
